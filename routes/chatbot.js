...
router.post('/ask', async (req, res) => {
  try {
    ...
    console.log('User message:', message, 'Language:', language, 'Session:', sessionId);
    ...
    if (!conversations.has(sessionId)) {
      conversations.set(sessionId, {
        messages: [],
        context: { lastTopic: null, location: null }
      });
    }
    ...
    if (isTravelRelated) {
      const fullConversation = [
        ...
        ...conversation.messages.slice(0, -1), // Previous messages
      ...
      if (message.length < 20 && conversation.context.lastTopic && conversation.context.isFollowUp) {
        ...
        fullConversation[fullConversation.length - 1].content = 
          `For ${message}, tell me about ${topic === 'cuisine' ? 'local foods and dishes' : topic}`;
      }
